  ...
  hermes:
    ...
    config:
      - key: hermes-video.default_detail
        ...
        default: "transcript-moments"
        prompt: "Default detail mode (transcript-moments|screenshot-first|transcript|efficient|balanced|token-burner)"
---
...
### Primary Workflow: Transcript-First with Agent-Driven Moment Selection

**This is the DEFAULT workflow.** Agent reads full transcript, selects key moments, extracts frames at those moments, then analyzes ALL frames comprehensively.

- [ ] Step 0: Setup preflight (`setup.py --json`)
- [ ] Step 1: Parse source + question from user input
- [ ] Step 2: Run `watch.py "<source>" --detail transcript-moments --min-moments 21 --out-dir <FIXED_DIR>`
...
- [ ] Step 3: Read full transcript from report.json
  - Use `jq -r '.transcript_segments[] | "[\(.start)] \(.text)"' <workdir>/report.json`
  - OR read `<workdir>/moments_prompt.txt` for formatted transcript
  - **MANDATORY: Read ALL transcript segments, not just a sample**
- [ ] Step 4: Select key moments using criteria below (minimum 21, no maximum)
  - Write moments as JSON to `<workdir>/key_moments.json`
  - Spread evenly across FULL duration (beginning, middle, AND end)
- [ ] Step 5: Re-run `watch.py` with same args including `--out-dir <FIXED_DIR>`
  - Video downloads + frames extracted at agent-selected timestamps
  - Or: use `background=true` for the re-run
- [ ] Step 6: Vision analyze ALL extracted frames (minimum 21, NO exceptions)
  - Analyze EVERY frame — never skip frames to "save API calls"
  - Use moment.question for each frame's vision_analyze call
- [ ] Step 7: Cross-reference transcript × visuals × metadata
  - Compare transcript claims vs visual evidence
  - Identify corrections (ASR errors, visual context)
  - Flag unverified claims
- [ ] Step 8: Generate comprehensive analysis (see Output Philosophy below)
- [ ] Step 9: Handle follow-ups from context, cleanup

### Moment Selection Criteria (MANDATORY)

Agent selects key moments using transcript + scene data. **Minimum 21 moments, no maximum — scale with video duration and content density.**

**Data sources for moment selection:**
- **Transcript segments**: Text content, timestamps, speaker transitions
- **Scene boundaries** (if available from report.json): Visual shifts, topic changes

**Selection criteria (apply ALL that match):**

1. **Proper nouns** — names, brands, titles, organizations that might be misspelled in auto-captions
2. **Claims/statistics** — numbers, prices, dates, percentages that need fact-checking
3. **Deictic references** — "ini", "itu", "lihat", "this", "that", "look at this" where speaker points at something
4. **Topic transitions** — moments where conversation shifts to a new subject
5. **Key arguments** — important conclusions, controversial statements, or strong opinions
6. **Visual context** — moments where understanding visuals changes interpretation (e.g., on-screen text, UI elements, demonstrations)
7. **Speaker identity** — moments where speaker changes or identity matters (multi-speaker videos)
8. **Entity recognition** — brand names, product names, on-screen text, logos

**Output format for key_moments.json:**
```json
[
  {
    "timestamp": 54.0,
...
**Rules:**
- `timestamp`: float seconds (NOT MM:SS string)
- `timestamp_fmt`: MM:SS string (agent MUST provide — this goes to `--timestamps`)
...
- **MINIMUM 21 moments required, NO MAXIMUM**

### Screenshot-First Mode (alternative for long videos with captions)
...
### Classic Mode (scene detection — for videos without captions)
...
- [ ] Step 2: Run `watch.py "<source>"` with appropriate flags
  - Short video (<10 min) → **foreground** `terminal(timeout=300)`
  - Long video (>10 min) → **background** `terminal(background=True, notify_on_complete=True)`
- [ ] Step 3: Wait for completion (**background mode only** — `process(action='wait')`)
- [ ] Step 4: Read full transcript from report.json (if available)
- [ ] Step 5: Vision analyze ALL extracted frames (minimum 21, NO exceptions)
- [ ] Step 6: Generate comprehensive analysis
- [ ] Step 7: Handle follow-ups from context, cleanup
...
**CRITICAL: Use background mode for long videos to avoid terminal timeout.** The `terminal()` tool has a default timeout of 180s (max foreground: 600s). Scene detection on videos >10 min can exceed this. Always use `background=true` + `notify_on_complete=true` for videos >10 min:

```
# Long videos (>10 min) — ALWAYS background:
terminal(
  command='python3 "${SKILL_DIR}/scripts/watch.py" "<source>" --detail transcript-moments --min-moments 21 --out-dir /tmp/watch-video',
  background=True,
...
```bash
python3 "${SKILL_DIR}/scripts/watch.py" "<source>" --detail transcript-moments --min-moments 21 --out-dir /tmp/watch-video
```

**Step 3 — read full transcript (MANDATORY).** After the script completes:
- Read `<workdir>/moments_prompt.txt` OR parse report.json with jq
- **MANDATORY: Read ALL transcript segments, not just a sample**
- Identify language from transcript content

**Step 4 — select key moments (minimum 21, no maximum).** Using the moment selection criteria:
- Apply all 8 criteria (proper nouns, claims, deictic references, etc.)
- Write moments as JSON to `<workdir>/key_moments.json`
- Spread evenly across FULL duration (beginning, middle, AND end)

**Step 5 — re-run watch script** with same `--out-dir` to extract frames at selected timestamps:
```bash
python3 "${SKILL_DIR}/scripts/watch.py" "<source>" --detail transcript-moments --min-moments 21 --out-dir /tmp/watch-video
```

**Step 6 — vision analyze ALL frames (NO exceptions).**
1. `search_files("*.jpg", path="<workdir>/frames", target="files")` to get frame list
2. Analyze EVERY frame with vision_analyze — never skip frames
3. Use moment.question for each frame's analysis question
4. Batch parallel calls when possible

**Step 7 — generate comprehensive analysis.** Cross-reference transcript × visuals × metadata. Deliver using the output format below.
...
**Step 5 — view frames.** The script outputs JPEG frame paths.

**Pitfall: frame filenames are NOT sequential from 0001.** Scene-change engines name files by extraction index (`frame_0211.jpg`), not timestamp. Do NOT guess filenames. Always `search_files("*.jpg", path="<workdir>/frames", target="files")` first.
...
**Frame Analysis Rule (MANDATORY — NO EXCEPTIONS):**
- **Analyze EVERY extracted frame with vision_analyze** — never skip frames to "save API calls"
- **Minimum 21 frames analyzed, NO maximum** — more frames = better analysis
- **Fewer frames = blind spots** — every skipped frame is a potential missed detail
- Use the moment's `question` field for each frame's vision_analyze call (from key_moments.json)
- If no key_moments.json, use specific questions based on transcript context at that timestamp
- **Batch parallel calls** — analyze multiple frames in the same turn when possible

**Step 6 — answer the user.** You now have two streams of evidence:
- **Frames** — what's on screen at each timestamp
- **Transcript** — what's said at each timestamp

**Cross-reference these streams** to identify:
- ASR errors (transcript says "X" but frame shows "Y")
- Speaker identity (who is speaking at each moment)
- Visual context (what's on screen when specific claims are made)
- Unverified claims (transcript mentions something not visible in frames)
...
**DO:**
- Summarize key arguments, main findings, conclusions, important quotes
- Include specific data (numbers, names, dates, statistics) from the transcript
- Provide context and background where relevant
- Match user language (if transcript is Indonesian, deliver in Indonesian)
- Structure for readability with clear sections

**DON'T:**
- Show work process (no cross-reference tables, correction sections, or frame-by-frame notes)
- Include stats block unless user specifically asks for it
- Fabricate any data not present in transcript or frames
- Skip important details to keep it "short"

**Analysis Quality Checklist (verify before delivering):**
...
- **User asked "summarize"** → comprehensive analysis with key findings, arguments, conclusions
- **No question** → thorough summary of what the video covers
...
**The user wants analysis, not process.** Deliver comprehensive content, not metadata.

Always use this structure when delivering watch results:
...
[Comprehensive analysis content — key findings, main arguments, conclusions, important quotes]
...
- **Stats block is OPTIONAL** — include ONLY if user specifically asks for processing stats
...
**Step 7 — follow-ups.** The downloaded video is auto-deleted. Frames and transcript remain in the working directory. Answer follow-ups from what you already have — do NOT re-run the script. If no follow-ups expected, clean up with `rm -rf <dir>`.

## Detail and frames

Default behavior comes from `~/.config/watch/.env`:

- `WATCH_DETAIL=transcript-moments|screenshot-first|transcript|efficient|balanced|token-burner` (default: `transcript-moments`)

**transcript-moments** — captions + agent-driven moment detection + frame extraction at 21+ timestamps (DEFAULT, recommended for comprehensive analysis). **screenshot-first** — LLM-driven section downloads; fastest for long videos with captions. **transcript** — captions only; no video download when captions exist. **efficient** — keyframes only (`ffmpeg -skip_frame nokey`), near-instant pass on scene cuts. **balanced** / **token-burner** — scene-aware frames with adaptive thresholding; balanced caps at 100, token-burner uses two-pass (scene detection + uniform sampling) and is uncapped. Frame report lines include timestamp and selection reason.
...
| Sections downloaded but extraction timed out | Manually extract from `<workdir>/sections/sec_*/video.mp4` | [partial-screenshot-first-recovery.md](references/partial-screenshot-first-recovery.md) |
