"""Tests for core module (src/watch/core.py).
...
class TestPipelineIntegration:
    """Integration tests for the full core."""
    ...
    def test_main_creates_report(self, cut_clip: Path, tmp_path: Path):
        ...
        out = _run(cut_clip, "--timestamps", "0.5", out_dir=out_dir)
        assert "frames" in out.lower()
    ...
    def test_main_balanced_detail(self, cut_clip: Path):
        ...
        out = _run(cut_clip, "--timestamps", "0.5,1.0")
        assert "frames" in out.lower()
        assert "frames" in out.lower()
...
def _write_captions_vtt(path: Path) -> Path:
    """Write a small VTT the core can parse into transcript segments."""
